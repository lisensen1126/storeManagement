...
export const root = process.env.API_ROOT || (
  location => `${location.protocol}//panel.${location.host.replace(/^.*?\./, '')}/v1`
)(window.location)
          ...
          parsing.then(
            result => {
              if (response.status === 401) {
                ...
                const consoleURIPattern = /^e\.([^.]+\.)?chedianai/
                location.href = process.env.LOGIN_PATH || `${location.protocol}//e.${(location.hostname.match(consoleURIPattern) || [])[1] || ''}chedianai.com/#/login`
