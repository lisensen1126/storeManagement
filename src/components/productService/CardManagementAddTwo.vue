...
<script>
  ...
  import ChooseCommodity from '@/components/productService/modal/ChooseCommodity'
  import ChooseServe from '@/components/productService/modal/ChooseServe'
  ...
  export default {
    ...
    methods: {
      ...
      async saveInfo () {
        ...
        let obj = {
          info: {
            ...
            is_image_customized: info.is_image_customized,
          },
          item: item,
          condition: condition,
        }
        console.log(obj, '添加提交数据')
        try {
          const {response, result} = await this.storeAddBossCardApi(obj)
      ...
      async updateInfo () {
        ...
        if (this.card.items) {
          arr.forEach(v => {
            let obj = {
              ...
              id: v.id,
              name: v.name,
              upkeep_info_id: v.upkeep_info_id,
            }
            item.push(obj)
          })
        }
        ...
        let condition = []
        // 所选的数据判断是否新的添加
        let oldContion = JSON.parse(sessionStorage.getItem('oldContion'))
        let oldIds = []
        oldContion.forEach(item => {
          oldIds.push(item.product_id)
        })
        console.log(oldContion, 'XXXXXX', this.data_list)
        this.data_list.forEach(item => {
          let temp = {
            sku_id: item.product_id,
            type: item.type,
          ...
          let index = oldIds.indexOf(item.product_id)
          if (index !== -1) {
            temp.id = oldContion[index].id
          }
          condition.push(temp)
        })
        let obj = {
        ...
        try {
          const {response, result} = await this.storeEditBossCardApi(obj)
          if (response.status === 200) {
            if (result.code && result.code !== 0) {
              this.$store.dispatch('showMessage', {
                type: 'failure',
    ...
    mounted () {
      let card = JSON.parse(sessionStorage.getItem('card'))
      this.temp_serves_list = card.temp_serves_list
      this.temp_goods_list = card.temp_goods_list
      console.log(this.temp_goods_list, this.temp_serves_list, 'XXXX')
      this.data_list = [...this.temp_serves_list, ...this.temp_goods_list]
      this.card = card || {}
    },
    activated () {
