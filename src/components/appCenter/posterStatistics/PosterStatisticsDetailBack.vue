<template lang="pug">
  //- 此页面为从门店，活动，节日海报海报的查看明细操作跳入
  .section-layout
    .section-filter.field-group.is-horizontal
      //- 如果tab是拼团秒杀养护卡则使用一个搜索框
      .field.keyword-wrapper
        label.label 员工姓名
        custom-input(v-model.trim="nickname", placeholder="请输入员工姓名", @keyupEnter="search")
      .field.keyword-wrapper
        label.label 手机号
        custom-input(v-model.trim="mobile", placeholder="请输入手机号", @keyupEnter="search")
      .field
        label.label 时间查询
  ...
  methods: {
    ...
    async getList () {
      try {
        ...
        data.nickname = this.queryData.nickname
        data.mobile = this.queryData.mobile
    ...
    search () {
      ...
      this.queryData.nickname = this.nickname
      this.queryData.mobile = this.mobile
      this.getList()
    },
    // 重置
    reset () {
    ...
    exportTable () {
      ...
      params = params + `nickname=${this.queryData.nickname}&`
      params = params + `mobile=${this.queryData.mobile}&`
