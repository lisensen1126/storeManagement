      ...
      .order-box(v-if="info.store")
        ...
        dl
          ...
          dd
            ...
            span(v-if="info.goods_type === 2")  养护卡
        ...
        table.table
          ...
          tbody
            tr(v-for="item in info.item")
              td
                .multi-content-td.has-image
                  .left-image(v-if="info.goods_type === 2")
              ...
              td
                ...
                .simple-td(v-if="info.order_type < 3 || info.goods_type === 2")
          ...
          dt.ctd
            div
              span(v-if="info.order_type === 3 || info.order_type === 4 ")
                ...
                span 总额：
              span(v-if="info.order_type < 3 || info.goods_type === 2") 商品总额：
...
export default {
  ...
  mixins: [ReportFormApi, imageScale, MathFun],
  computed: {
  ...
  methods: {
    ...
    async getInfo () {
      try {
        const { response, result } = await this.OrderSplitDetailsApi(this.tradeOrderId)
