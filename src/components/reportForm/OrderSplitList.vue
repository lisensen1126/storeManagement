  ...
  .section-layout
    ...
    .section-filter.is-horizontal(v-if="tab_select.type == 3")
      .field-group.is-horizontal
        ...
        .field
          .space-text(style="line-height: 32px") 至
        .field
          label.label
          .field.is-horizontal
            el-date-picker(
      ...
      .choose-content
        transition(name="sliding")
          .batched-operations(v-if="selected_boss.length>0")
            .batched-operation-segment.batched-operation.is-secondary.btn-cover.blue(@click="cannelChoose") 取消选择
        ...
        table.table
          ...
          tbody(v-if="list.length")
            tr(v-for="(item, i) in list")
              ...
              td
                .simple-td(v-if="item.order_type === 1") 普通
                .simple-td(v-if="item.order_type === 2") 智能保养
                .simple-td(v-if="item.order_type === 3") 拼团
                .simple-td(v-if="item.order_type === 4") 秒杀
                .simple-td(v-if="item.goods_type === 2") 养护卡
              td
                .simple-td {{item.is_separate==2?'未分账':item.is_separate==1?'已分账':''}}
              td
                .simple-td {{item.created_at | formatDate}}
              td
                .simple-td.width-160 {{(order_source_list[item.purchase_type-1]||'').name||''}}
              td.operation-td
                router-link.operation-link(:to="{name: 'orderSplitDetail', params: {id: item.trade_order_id}}") 查看
      pagination(:pagination-data="page_data", @input="paging")
</template>
...
<script>
  ...
  export default {
    ...
    mixins: [ReportFormApi, imageScale, getYearMonth, MathFun],
    computed: {
      tab_list () { // 头部的tab
