...
<script>
  ...
  export default {
    name: 'GroupGoodAdd',
    mixins: [productServiceMixin, fileUploader, imageScale, spikeApi, FormateDate],
    components: {
      CustomInput,
      UploadPic,
      OpenImage,
      ...
      inspectData () {
        ...
        let params = {
          ...
          product: {
            ...
            basic_spu_id: this.info.basic_spu_id,
        ...
        this.sku_data.forEach(item => {
          let v = {
            sku_id: item.sku_id,
            basic_sku_id: item.basic_sku_id,
            value: item.name,
        ...
        try {
          ...
          if (response.status === 200) {
            ...
            sku.forEach(v => {
              ...
              v.limit_price = v.limit_price === 0 ? '' : (v.limit_price / 100).toFixed(2)
              v.sell_price = (v.sell_price / 100).toFixed(2)
              v.inventory_total = (v.inventory_total + v.sales_total === 0) ? '' : v.inventory_total + v.sales_total
            })
            this.sku_data = sku
          } else {
            this.$store.dispatch('showMessage', {
