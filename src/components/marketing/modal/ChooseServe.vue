      ...
      .modal-body
        .section-filter.is-horizontal
          .field-group.is-horizontal
            .field.is-narrow
              label.label 服务名称
              custom-input(v-model="filter_data.keywords", placeholder="请输入名称", @keyupEnter="searchFilter()")
            .field.is-narrow
              label.label 全部分类
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        first_class_list: [], // 服务一级分类
        second_class_list: [], // 服务二级分类
        current_id: '',
        is_initiated: false,
        filter_data: { // 搜索条件
          brand_id: '',
    ...
    methods: {
      ...
      async getServeData () {
        ...
        } else if (this.query_filter_data.first_state && this.query_filter_data.first_state.category_id) {
        ...
        try {
          const {response, result} = this.type === 'group' ? await this.groupServeApi(params) : await this.serviceListApi(params)
          if (response.status === 200 && result.code === 0) {
            result.data.forEach(v => {
              v.checked = false
              // 如果之有sell_price则为拼团api的返回需要对sell_price除100,秒杀则没有此字段
              if (v.sell_price) {
