      ...
      .modal-body
        .section-filter.is-horizontal
          .field-group.is-horizontal
            ...
            .field.staff-wrapper(v-if="show_tab")
              label.label 全部分类
              choose-tab(@success="getClassify" ref="child" :default="query_data.search_classify")
            .field
              a.button.primary(@click="applyFilter") 搜索
            .field
              a.button(@click="resetFilter") 重置
        empty-page(v-if="!data_list.length && is_initiated", :tip="'暂无秒杀'")
          ...
          table.table
            ...
            tbody
              tr(v-for="(item, index) in data_list",@click="setChecked(index, item)")
                ...
                td
                  .simple-td(v-text="item.product.name")
                td
                  .simple-td(v-text="item.product.category_name")
                //- td
...
<script>
  ...
  export default {
    ...
    mixins: [ModalInteractions, Api, ImageScale, FilterTools, GroupApi, spikeApi],
    props: {
      id: {
        type: [String, Number],
      },
    ...
    methods: {
      // 获取分类
      getClassify (val) {
        this.query_data.search_classify = val
      },
      applyFilter () {
