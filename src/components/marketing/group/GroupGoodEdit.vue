...
<script>
  ...
  export default {
          ...
          if (response.status === 200) {
            if (result.code === 0) {
              this.group_num.forEach(item => {
                if (item.id === result.data.num) this.group.num = item
              })
              this.group_time.forEach(item => {
                if (item.id === result.data.time) this.group.time = item
              })
              let arr = []
              data.forEach(item => {
              ...
              result.data.product_sku.forEach(item => {
                if (arr.indexOf(item.basic_sku_id) !== -1) {
                  data[arr.indexOf(item.basic_sku_id)].group_price = item.group_price === 0 ? '' : (item.group_price / 100).toFixed(2)
    ...
    mounted () {
      this.product_id = this.$route.query.basic_spu_id
