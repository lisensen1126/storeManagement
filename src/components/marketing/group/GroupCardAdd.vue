  ...
  export default {
    ...
    methods: {
      ...
      async addGroup () {
        ...
        try {
          const {response, result} = await this.addCardGroupApi(params)
          if (response.status === 200) {
            if (result.code === 0 && result.message === 'success') {
              // this.changeApi(this.$route.query.group_id, 1)
              this.$store.dispatch('showMessage', {
      ...
      async getCardDetail () {
        try {
          ...
          if (response.status === 200 && result.code === 0) {
            ...
            this.type = this.detail.info.type - 1
