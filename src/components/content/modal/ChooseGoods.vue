...
<script>
  ...
  export default {
    ...
    mixins: [ModalInteractions, Api, productServiceMixin, ImageScale, FilterTools, MathFun],
    props: {
      array: {
        type: Array,
        default: () => [],
      ...
      async getGoodsData () {
        ...
        try {
          ...
          if (response.status === 200 && result.code === 0) {
            ...
            this.data_list.forEach(ele => {
              ele.type = 1
              ele.mini_price = self.accDiv(ele.mini_price, 100).toFixed(2)
            })
            // 分页器数据更新
