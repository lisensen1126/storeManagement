      ...
      .modal-body
        .section-filter.is-horizontal
          .field-group.is-horizontal
            .field.is-keyword
              label.label 卡名
              custom-input(v-model="filter_data.keywords", placeholder="请输入卡名", @keyupEnter="searchFilter()")
            .field
...
<script>
  ...
  export default {
    ...
    mixins: [ModalInteractions, Api, productServiceMixin, ImageScale, FilterTools],
    props: {
      array: {
        type: Array,
        default: () => [],
    ...
    methods: {
      ...
      async getGoodsData () {
        ...
        try {
          const {response, result} = await this.upkeepListApi(params)
    ...
    mounted () {
      ...
      console.log(this.selected_item, 'ddd')
    },
  }
</script>
