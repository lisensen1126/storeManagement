...
export default {
  ...
  methods: {
      ...
      this.uploadImage(file, this.ImgType).then(({ result }) => {
        ...
        if (result.code === 0) {
          if (this.ImgType - 0 === 1) {
            ...
            this.$emit('input', result.data.local.url)
    ...
    // 解析图片方法
    async transformImg (img) {
      let _this = this
      let params = {
        url: img,
      }
      try {
        const { response, result } = await _this.exchangeImageUrlApi(params)
  ...
  watch: {
    value (val) {
      if (this.isMeituan) {
        ...
        if (val) {
          this.transformImg(val).then(({ result }) => {
