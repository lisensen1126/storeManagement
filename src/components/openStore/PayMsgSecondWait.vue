<template lang="pug">
  .section-layout
    ...
    .content-wrapper
      open-store-title(title="支付申请二：支付账号验证审核")
      .form-wrapper
        .warn-box
          img.warn-icon(v-if="is_verify_card === 3",src="../../assets/modal/warning.svg")
          img.warn-icon(v-if="is_verify_card && is_verify_card !== 3", src="../../assets/modal/success.svg")
          .warn-content(v-if="is_verify_card === 1")
            .warn-title 您还未提交验证，请提交支付账号验证
          .warn-content(v-if="is_verify_card === 2")
            .warn-title 资料已提交成功，您的美团支付账号正在审核中，请耐心等待。
            .warn-text 正在审核，请耐心等待，审核成功后可进行下一步操作。
            .warn-text 注：若您想更快的看到审核结果，可刷新当前页面。
          .warn-content(v-if="is_verify_card === 3")
            .warn-title 支付账号验证审核已通过
          .warn-content(v-if="is_verify_card === 4")
            .warn-title 审核失败，请重新提交资料。
            .warn-text.err 失败原因: {{reason}}
      .btn-wrapper(v-if="is_verify_card != 2")
        .button-group
          a.button.primary.large.width-200(v-if="is_verify_card === 1", @click="goPayMsgSecond") 申请支付账号验证
          a.button.primary.large.width-150(v-if="is_verify_card === 3", @click="goPayMsgThird") 支付配置申请
          a.button.primary.large.width-150(v-if="is_verify_card === 4", @click="goPayMsgSecond") 重新提交
</template>
...
export default {
  ...
  data () {
    return {
      is_verify_card: 0, // 是否验证银行卡：1 等待提交验证，2 验证中，3 验证成功，4 验证失败
      reason: '', // 失败原因
    }
  },
  methods: {
    ...
    async merchantStore () {
      try {
        ...
        if (response.status === 200 && result.code === 0 && (result.data.is_verify_card === 1 || result.data.is_verify_card === 2 || result.data.is_verify_card === 3 || result.data.is_verify_card === 4)) {
          this.is_verify_card = result.data.is_verify_card
          this.reason = result.data.last_bank_card ? result.data.last_bank_card.reason : ''
        } else {
