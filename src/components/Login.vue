<template lang="pug">
  .login-wrapper
    ...
    .copyright-wrapper © {{year}} chedianai.com. All rights reserved.
</template>
...
export default {
    ...
    async checkIn () {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          sessionStorage.setItem('store-info', JSON.stringify(result.data.store))
  ...
  mounted () {
    this.$store.dispatch('clearStores') // 每次登录，清除门店列表缓存，调接口拉取新的门店列表数据
    let now = new Date()
