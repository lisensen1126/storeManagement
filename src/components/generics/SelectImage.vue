      ...
      .modal-body
        .filter-wrapper
          .upload-pic(v-for="(item, index) in imageArray")
            upload-img-model(v-model="imageArray[index]", img-type="imageArray", :has-second-action="true", @other="deleteImage", need-move="2", @leftMove="leftPic('item', index)", @rightMove="rightPic('item',index)", :need-left="index !== 0", :need-right="index !== (imageArray.length -1)")
