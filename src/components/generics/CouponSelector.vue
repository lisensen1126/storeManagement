<template lang="pug">
  modal-dialog(modal-scale="large", v-if="value")
    .modal-card(slot="card")
      .modal-header
        .modal-card-title 选择优惠券
        .close-button(@click="modalClose")
      .modal-body
        .filter-wrapper
          custom-input.custom-input(placeholder="搜索优惠券", v-model="filterData.keyword", @keyupEnter="applyFilter")
          a.button.primary(@click="applyFilter") 搜索
          a.button(@click="resetFilter") 重置
        empty-page(v-if="!dataList.length && isInitiated", :tip="'暂无优惠券'")
        .table-wrapper(v-if="dataList.length")
          .items(v-for="item in dataList")
            ...
            .item-content
              ...
              span.amount(v-text="'￥' + item.preferential_price")
              span.condition(v-if="item.type === 'DEDUCTION'") 无限制
              span.condition(v-if="item.type !== 'DEDUCTION'") 满{{item.condition_value}}可用
              span.use-scope(v-if="item.use_range.type === 'COMMON'") 通用
              span.use-scope(v-if="item.use_range.type === 'WASHING'") 洗车
              span.use-scope(v-if="item.use_range.type === 'SHOP_CATEGORY' && item.use_range.value === 'ALL'") 服务商城
              span.use-scope(v-if="item.use_range.type === 'SHOP_CATEGORY' && item.use_range.value !== 'ALL'", v-text="'服务商城 - ' + item.use_range.value_name + '分类'")
        .pagination
...
import modalApi from '@/mixins/modules/marketing'
    ...
    async getFullItems () {
      ...
      try {
        const {response, result} = await this.fetchCoupons({
          keyword: this.queryData.keyword || undefined,
          not_expired: true,
