<template lang="pug">
  .control(:class="[{ 'is-error': validator && validator.$error, 'is-disabled': isDisabled}, scaleClass]")
    textarea.textarea(:value="value", @input="updateValue($event.target.value)", @blur="check($event.target.value)", :placeholder="placeholder", :disabled="isDisabled", :maxlength="maxlength", :type="type", :max="max", :min="min", :class="{'is-disabled': isDisabled}")
