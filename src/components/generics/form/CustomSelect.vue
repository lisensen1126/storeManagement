<template lang="pug">
  .custom-select
    .custom-select-area(@click.stop="toggleDropdown", :class="{'is-disabled': isDisabled, 'is-error': validator && validator.$error}")
      input.custom-select-input(:placeholder="placeholder", readonly="readonly", type="text", :disabled="isDisabled", :title="value && value[nameField]", :value="value && value[nameField]", :class="{'is-disabled': isDisabled}", :style="'color:'+ textColor + '!important;'")
    ...
    .custom-select-list(v-if="isExpanded")
      input.custom-select-input.custom-select-search(v-if="showSearch", v-model="searchKey", :placeholder="'快速检索'", type="text",@focus="changeInputFlag(1)",@blur="changeInputFlag(0)",  :style="'color:'+ textColor + '!important;'")
