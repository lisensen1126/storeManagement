...
<template lang="pug">
  modal-dialog(modal-scale="large")
    .modal-card(slot="card")
      ...
      .modal-footer.level
        .level-left
          span(v-if="!callback_data.category_name || callback_data.category_name ==='服务分类'", v-text="'未选'")
          span(v-if="callback_data.category_name && callback_data.category_name !=='服务分类'", v-text="'已选：' + callback_data.category_name")
        .level-right
          a.button.large(@click="modalClose") 取消
          a.button.large.primary(@click="complete" v-show="callback_data.category_name && callback_data.category_name !=='服务分类'") 确定
</template>

<script>
  import ModalDialog from '@/components/generics/ModalDialog'
  import ModalInteractions from '@/mixins/utilities/modalInteractions'
  import BossMixin from '@/mixins/modules/productService'
    ...
    methods: {
      ...
      loadNode (node, resolve) {
        ...
        if (node.level === 0) {
          ...
          _this.parant_id = 0
          return resolve([{ category_name: '服务分类', category_ischeck: 1 }])
        }
      ...
      async fetchData () {
        ...
        try {
          const {response, result} = await this.commodityClassification(this.submit_data)
