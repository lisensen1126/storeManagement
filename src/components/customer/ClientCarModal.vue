        ...
        .tab-content-item(v-if="currentTab && currentTab.data")
          .tab-content-block(v-for="item in currentTab.data.category")
            ...
            .item-content(v-for="category_item in item.category_item")
              ...
              .profiled.flex.between(v-if="category_item.exception_video && category_item.exception_video.length > 0")
                .labeltitle 视频:
              .profiled.flex.imgbox(v-if="category_item.exception_video && category_item.exception_video.length > 0")
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async fetchDetail () {
        try {
          const { response, result } = await this.fetchInspectDetailApi({
            id: this.id,
          })
