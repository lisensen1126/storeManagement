pipeline {
  ...
  environment {
    ...
    // 项目名
    DOCKER_PROJECT_NAME = 'enterprise_management'
  }
  options {
      //保持构建的最大个数
          ...
          if ("${env.needDeploy}".contains("Yes")) {
            ...
            withCredentials([kubeconfigContent(credentialsId: env.k8sConfigId, variable: 'KUBECONFIG_CONTENT')]) {
                sh '''
                    ...
                    kubectl set image deployment/enterprise-management enterprise-management=${onlineImage} --namespace=${k8sNamespace} --kubeconfig=kubeconfig --insecure-skip-tls-verify=true
                    rm kubeconfig
                '''
            }
            echo "部署服务器完成。。。"
